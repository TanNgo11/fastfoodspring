...
<body>
								...
								<div class="container">
									<form:form modelAttribute="product"
										...
										<div class="form-row">
											...
											<div style="width: 99% !important" " class="form-group file">
												<div class="row">
													<c:set var="imageUrl1" value="" />
													<c:if
														test="${not empty product.getListImage() and product.getListImage().size() > 0}">
														<c:set var="imageUrl1"
															value="${product.getListImage().get(0).getImageURL()}" />
													</c:if>
													<div style="padding-left: 22px;" class="col-md-4">
														<label for="exampleFormControlFile1">Product Image
															1</label>
														<div class="drag-image " data-image-url="${imageUrl1}"
															data-index="0">
															<div class="icon">
													...
													<div style="padding-left: 22px;" class="col-md-4">
														<label for="exampleFormControlFile1">Product Image
															2</label>
														<c:set var="imageUrl2" value="" />
														<c:if
															test="${not empty product.getListImage() and product.getListImage().size() > 1}">
															<c:set var="imageUrl2"
																value="${product.getListImage().get(1).getImageURL()}" />
														</c:if>
														<div class="drag-image " data-image-url="${imageUrl2}"
															data-index="1">
															<div class="icon">
																<i class="fas fa-cloud-upload-alt"></i>
															</div>
													...
													<div style="padding-left: 22px;" class="col-md-4">
														<label for="exampleFormControlFile1">Product Image
															3</label>
														<c:set var="imageUrl3" value="" />
														<c:if
															test="${not empty product.getListImage() and product.getListImage().size() > 2}">
															<c:set var="imageUrl3"
																value="${product.getListImage().get(2).getImageURL()}" />
														</c:if>
														<div class="drag-image " data-image-url="${imageUrl3}"
															data-index="2">
															<div class="icon">
																<i class="fas fa-cloud-upload-alt"></i>
															</div>
 ...
 -->
	...
	<script src="/template/js/dragProductImage.js"></script>
	<script>
		...
		function updateProduct(formData, id) {
			...
			$.ajax({
				...
				success : function(result) {
					window.location.href = "/admin/product/edit?id="
							+ result.id + "&msg=success_update";
				},
				error : function(error) {
					console.log(error)

					window.location.href = "/admin/product/edit?msg=error_update";
				}
			});
		}
